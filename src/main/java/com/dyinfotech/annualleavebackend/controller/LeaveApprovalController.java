...
public class LeaveApprovalController {
    ...
    @GetMapping("/approved")
    public List<LeaveRequestListDto.LeaveRequestListResponse> getApprovedRequests(@RequestParam(value = "team", required = false) String team, @AuthenticationPrincipal Long employeeId) {
    	return leaveApprovalService.getApprovedRequests(employeeId, team);
    }
    
    @Operation(summary = "반려 상태 휴가 조회", description = "관리자가 하위팀의 반려 상태인 전체 휴가 정보를 조회한다.")
    @GetMapping("/rejected")
    public List<LeaveRequestListDto.LeaveRequestListResponse> getRejectedRequests(@RequestParam(value = "team", required = false) String team, @AuthenticationPrincipal Long employeeId) {
        return leaveApprovalService.getRejectedRequests(employeeId, team);
    }
